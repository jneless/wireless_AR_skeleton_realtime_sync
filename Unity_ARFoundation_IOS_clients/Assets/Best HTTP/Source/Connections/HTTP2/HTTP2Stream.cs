...
namespace BestHTTP.Connections.HTTP2
{
    ...
    {
        private readonly TimeSpan CloseStreamAfter = TimeSpan.FromSeconds(30);
        ...
        /// </summary>
        public HTTP2Stream(UInt32 id, HTTP2SettingsManager registry, HPACKEncoder hpackEncoder)
        {
                ...
                switch (frame.Type)
                {
                    ...
                    case HTTP2FrameTypes.CONTINUATION:
                        ...
                        if ((frame.Flags & (byte)HTTP2HeadersFlags.END_HEADERS) != 0)
                        {
                            ...
                            {
                                HTTPManager.Logger.Exception("HTTP2Stream", string.Format("[{0}] ProcessIncomingFrames - Header Frames: {1}, Encoder: {2}", this.Id, this.currentFramesView.ToString(), this.encoder.ToString()), ex);
                            }
                            ...
                            if (frame.Type == HTTP2FrameTypes.HEADERS && (frame.Flags & (byte)HTTP2HeadersFlags.END_STREAM) != 0)
                            {
                                PlatformSupport.Threading.ThreadedRunner.RunShortLiving<HTTPRequest, HTTP2Response, FramesAsStreamView>(FinishRequest, this.AssignedRequest, this.response, null);
