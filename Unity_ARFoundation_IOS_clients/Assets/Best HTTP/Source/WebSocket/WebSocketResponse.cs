...
namespace BestHTTP.WebSocket
{
    public sealed class WebSocketResponse : HTTPResponse, IHeartbeat, IProtocol
    {
                ...
                using (var bufferedStream = new ReadOnlyBufferedStream(this.Stream))
                {
                    while (!closed)
                    {
                        try
                        {
                            ...
                            if (frame.HasMask)
                            {
                                HTTPManager.Logger.Warning("WebSocketResponse", "Protocol Error: masked frame received from server!");
        ...
        void IProtocol.HandleEvents()
        {
                ...
                try
                {
                    switch (frame.Type)
                    {
                        ...
                        case WebSocketFrameTypes.Text:
                            ...
                            HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - OnText");
                            if (OnText != null)
                        ...
                        case WebSocketFrameTypes.Binary:
                            ...
                            HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - OnBinary");
                            if (OnBinary != null)
            ...
            {
                HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - Calling OnClosed");
                try
                {
                    UInt16 statusCode = 0;
