...
namespace UnityEngine.XR.ARFoundation.Samples
{
    public class BoneController : MonoBehaviour
    {
        ...
        public void ApplyBonePoseFromServer(string data)
        {
            string[] lines = data.Split('\n');
            int i = 0;
            for (i = 0; i < k_NumSkeletonJoints; ++i)
            {
                var bone = m_BoneMapping[i];
                if (bone != null)
                {
                    string[] xyz = lines[i*2].Split(',');
                    bone.transform.localPosition = new Vector3(float.Parse(xyz[0]), float.Parse(xyz[1]), float.Parse(xyz[2]));
                    xyz = lines[i*2 + 1].Split(',');
